...
class Dashboard extends CI_Controller
{
	...
	public function movement_bill_insert1()
	{
		...
		//if ($fplace != '' && $ftime != '' && $tplace != '' && $ttime != '' && $purpose != '' && $mode != '' && $taka != '') {
			for ($i = 0; $i < count($mode); $i++) {
				$data["i"] = $i;
				//$data["userid"] = $userid;
//				$data["sdate"] = date("Y-m-d", strtotime($sdate));
//				$data["cdate"] = date("Y-m-d", strtotime($cdate[$i]));
//				$data["factory"] = $factory[$i];
				//$data["billfor"] = $billfor[$i];
				$data["mtoken"] = $mtoken;
