	...
	public function user_insert()
	 {
		...
		if($this->input->post('submit'))
		{
			...
			$ins=$this->Admin->user_insert($factoryid,$departmentid,$designationid,$name,$email,$pmobile,$userid,$password);
            		if($ins==TRUE)
						{
							$this->session->set_flashdata('Successfully','Successfully Inserted/Contact Now IT Dept.');
						}
					else
						{
							$this->session->set_flashdata('UnSuccessfully','Failed To Inserted/Already Inserted');
						}
					redirect('User_Login');
				}
