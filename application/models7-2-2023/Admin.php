...
class Admin extends CI_Model
{
	...
	public function movement_list($userid)
	{
		...
		JOIN line_manager_details ON line_manager_details.lmuserid=movement_insert.lmauserid
		WHERE movement_insert.userid='$userid' AND mstatus!='4'";
		$result = $this->db->query($query);
		return $result->result_array();
	}
...
//	}
	...
	public function movement_bill_update($mtoken,$transport,$tiffin,$accomodation,$gift,$others)
	{
		$sql = "UPDATE movement_bill_insert SET transport='$transport',tiffin='$tiffin',accomodation='$accomodation',gift='$gift',others='$others' WHERE mtoken='$mtoken'";
		return $query = $this->db->query($sql);
	}
	//public function approval_list($lmauserid)
//	{
//		$query = "SELECT SUM(transport+tiffin+accomodation+gift+others) AS ctotal,transport,tiffin,accomodation,gift,others,mid,user.userid,name,movement_insert.lmauserid,fdate,tdate,ftime,ttime,purpose,location,lname
//		FROM movement_insert
//		JOIN user ON user.userid=movement_insert.userid
//		JOIN movement_bill_insert ON movement_bill_insert.mtoken=movement_insert.mid
//		JOIN line_manager_details ON line_manager_details.lmuserid=movement_insert.lmauserid
//		WHERE movement_insert.lmauserid='$lmauserid'
//		GROUP BY mtoken";
//		$result = $this->db->query($query);
//		return $result->result_array();
//	}

	public function approval_pending_list($userid)
	{
		$query = "SELECT * FROM approval_details
		WHERE lmauserid='$userid' AND mstatus='2'";
		$result = $this->db->query($query);
		return $result->result_array();
	}
	public function movement_bill_approved($mtoken)
	{
		$sql = "UPDATE movement_insert SET mstatus='3' WHERE mid='$mtoken'";
		return $query = $this->db->query($sql);
	}
	public function date_wise_approval_list($pd,$wd,$userid)
	{
		...
		$query = "SELECT * FROM approval_details
		WHERE fdate BETWEEN '$pd' AND '$wd' AND lmauserid='$userid'";
		$result = $this->db->query($query);
		return $result->result_array();
	}
	...
	{
		$query = "SELECT * FROM approval_details WHERE mid='$mid'";
		$result = $this->db->query($query);
